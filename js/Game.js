class Game {
  ...
  init() {
    ...
    for (let i = 0; i < BIRD_POPULATION; i++) {
      if (i === 0) {
        this.birds[i] = new Bird(INITIAL_BIRD_X, INITIAL_BIRD_Y, this.sling);
    ...
    for (let i = 0; i < OBSTACLE_POPULATION; i++) {
      this.obstacles[i] = new Obstacle(
        ...
        OBSTACLE_POSITION.y[i] - obstacleImageType['stone']['height'],
        'stone'
      );
    }
  ...
  startGameLoop() {
    ...
    if (
      this.defeatedBirds.length === BIRD_POPULATION &&
  ...
  handleCollisions() {
    ...
    if (this.birds.length != 0) {
      ...
      for (let pig of this.pigs) {
        for (let obstacle of this.obstacles) {
          if (
            checkCircleToRectangleCollision(this.birds[0], obstacle) &&
    ...
    if (this.birdInAir && this.birdInAir.collision) {
